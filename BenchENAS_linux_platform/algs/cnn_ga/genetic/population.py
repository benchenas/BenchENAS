...
class Individual(object):
    ...
    #随机生成一个conv+pool的个体（随机组合卷积层和池化层的顺序）
    # num_conv = 8
    # num_pool = 4
    # [6, 0, 7, 1, 3, 2, 5, 4]
    # [0 1 6 7]
    ...
    def initialize(self):
        ...
        availabel_positions = list(range(num_conv))  #0-num_conv
        np.random.shuffle(availabel_positions) #随机排序列表
        select_positions = np.sort(availabel_positions[0:num_pool])  #list 选择池化层插入的位置
        all_positions = []
        #分配卷积层和池化层的位置
