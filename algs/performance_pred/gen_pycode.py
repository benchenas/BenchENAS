...
class GenPyModel(object):
    def __init__(self):
        ...
        self.conv_pat = re.compile(
            '(\d+),CONV,in:([\d-]+),out:([\d-]+),kernel:([\d-]+),stride:([\d-]+),groups:(\d+),padding:(\d+),add_to:([-\dNone]+),cat_to:([-\dNone]+)')
        self.avgp_pat = re.compile(
            '(\d+),AVG_POOL,in:([\d-]+),out:([\d-]+),kernel:([\d-]+),stride:([\d-]+),padding:(\d+)')
        self.maxp_pat = re.compile(
            '(\d+),MAX_POOL,in:([\d-]+),out:([\d-]+),kernel:([\d-]+),stride:([\d-]+),padding:(\d+)')
                ...
                if l_type == 'conv':
                    ...
                    if ic > self.max_cat and dpc_list[i] > 0:
                        ...
                        for fpos in dsp_list[i]:
                            ...
                            layer_init += '\n%sself.adjust%d_%d = nn.Conv2d(%d, %d, (1, 1), (1, 1))' % (
                            space_str, fpos, pos, fpos_org_c, adjust_num)
                            adjust_cout += adjust_num
                        org_in_c = l['in'][2]
                        ic_another_way += org_in_c
                        adjust_in_c = int(org_in_c * adjust_rate)
                        layer_init += '\n%sself.adjust%d = nn.Conv2d(%d, %d, (1, 1), (1, 1))' % (
                        space_str, pos, org_in_c, adjust_in_c)
                        ...
                        layer_init += '\n%sself.conv%d = nn.Sequential(nn.Conv2d(%d, %d, (%d, %d), (%d, %d), padding=%d, groups=%d, bias=False), nn.BatchNorm2d(%d))' \
                                      % (space_str, pos, adjust_cout, oc, kernel[0], kernel[1], stride[0], stride[1],
                                         padding, groups, oc)

                        before_i[i] = adjust_cout
                    else:
                        layer_init = '\n%sself.conv%d = nn.Sequential(nn.Conv2d(%d, %d, (%d, %d), (%d, %d), padding=%d, groups=%d, bias=False), nn.BatchNorm2d(%d))' \
                                     % (space_str, pos, ic, oc, kernel[0], kernel[1], stride[0], stride[1], padding,
                                        groups, oc)
                        before_i[i] = ic
                    ...
                    for fpos in scp_list[pos]:
                        ...
                        if total_c != front_out_c:
                            # use auxiliary conv
                            layer_init += '\n%sself.side_%d_%d = nn.Conv2d(%d, %d, (1, 1), (1, 1))' % (
                            space_str, fpos, pos, total_c, front_out_c)
                            layer_forward += '\n%sx%d = self.side_%d_%d(x%d) + x%d' % (
                            space_str, pos - 1, fpos, pos, fpos, pos - 1)
                        else:
                    ...
                    for fpos in dsp_list[pos]:
                        if ic > self.max_cat and dpc_list[i] > 0:
                            # adjust the channel
                            layer_forward += '\n%sx%d = torch.cat([x%d, self.adjust%d_%d(x%d)], dim=1)' % (
                            space_str, pos - 1, pos - 1, fpos, pos, fpos)
                        else:
                            layer_forward += '\n%sx%d = torch.cat([x%d, x%d], dim=1)' % (
                            space_str, pos - 1, pos - 1, fpos)
                    if i == 0:
                    ...
                    layer_init = '\n%sself.%s%d = nn.%s((%d, %d), (%d, %d), padding=%d)' \
                                 % (space_str, l_type, pos, 'AvgPool2d' if l_type == 'avgp' else 'MaxPool2d', kernel[0],
                                    kernel[1], stride[0], stride[1], padding)
                    ...
                    if i == 0:
                        ...
                        layer_forward += '\n%sx%d = F.relu(self.linear%d(x%d), inplace=True)' % (
                        space_str, pos, pos, pos - 1)
