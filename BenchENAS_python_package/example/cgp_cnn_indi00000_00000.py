...
class Concat(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        if (in_data[0].size(2) - in_data[1].size(2)) != 0:
            small_in_id, large_in_id = (0, 1) if in_data[0].size(2) < in_data[1].size(2) else (1, 0)
            pool_num = math.floor(math.log(in_data[large_in_id].size(2) / in_data[small_in_id].size(2), 2))
...
class EvoCNNModel(nn.Module):
    def __init__(self):
        ...
        cgp = [['input', 0, 0], ['S_ConvBlock_32_3', 0, 0], ['S_ConvBlock_32_3', 1, 1], ['S_ConvBlock_64_5', 2, 0], ['Max_Pool', 3, 2], ['S_ConvBlock_64_5', 4, 3], ['S_ResBlock_32_3', 5, 5], ['full', 6, 6]]
        in_channel = 3
        n_class = 10
        imgSize = 32
