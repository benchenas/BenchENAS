...
class Utils(object):
    ...
    def save_population_at_begin(cls, _str):
        file_name = '%s/begin_.txt' % (os.path.join(get_algo_local_dir(), 'populations'))
    ...
    def generate_pytorch_file(cls, indi, params, test=False):
        ...
        if Config_ini.dataset == 'MNIST':
            s2 = 'self.net = NetworkMNIST(' + str(StatusUpdateTool.get_input_channel()) + ', ' + str(params['F']) \
                 + ', ' + str(StatusUpdateTool.get_num_class()) + ', ' + str(params['N']) + ', True, genotype)'
        elif Config_ini.dataset.startswith('CIFAR'):
            s2 = 'self.net = NetworkCIFAR(' + str(StatusUpdateTool.get_input_channel()) + ', ' + str(params['F']) \
                 + ', ' + str(StatusUpdateTool.get_num_class()) + ', ' + str(params['N']) + ', True, genotype)'
        else:
            s2 = 'self.net = NetworkImageNet(' + str(StatusUpdateTool.get_input_channel()) + ', ' + str(params['F']) \
                 + ', ' + str(StatusUpdateTool.get_num_class()) + ', ' + str(params['N']) + ', True, genotype)'
