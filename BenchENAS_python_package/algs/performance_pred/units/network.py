...
class CNN():
    ...
    def init_conv(self, _no, _in_size):
        ...
        conv = ConvUnit()
        conv.create(no=_no, in_size=_in_size, out_size=out_size, kernel_size=[kernel_size, kernel_size],
                    stride_size=[stride_size, stride_size], groups=groups, padding=padding, add_to=None,
                    concatenate_to=None)
        return conv
    ...
    def init_pool(self, _no, _in_size):
        ...
        if pool_type == 0:
            ...
            pool = AvgPool()
        pool.create(no=_no, in_size=_in_size, kernel_sizde=[kernel_sizde, kernel_sizde],
                    stride_size=[stride_size, stride_size], padding=padding)
        return pool
    ...
    def __str__(self):
        _str = []
        s1 = 'id:%s,depth:%d,conv:%d,pool:%d,full:%d,uuid:%s' % (
        self.id, self.network_depth, self.conv_unit_size, self.pool_unit_size, self.full_unit_size, self.uuid())
        _str.append(s1)
        for u in self.units:
            _str.append(str(u))
        return '\n'.join(_str)
