            ...
            if p_ < self.prob:  # the probability of crossover operation
                ...
                self.log.info(
                    'Change the input channel of unit at %d to %d that is the output channel of unit at %d in %s' % (
                        j, last_output_from_list1, i, parent1.id))
                ...
                self.log.info(
                    'Change the input channel of unit at %d to %d that is the output channel of unit at %d in %s' % (
                        j, last_output_from_list2, i, parent2.id))
...
class Mutation(object):
    ...
    def do_mutation(self):
        _stat_param = {'offspring_new': 0, 'offspring_from_parent': 0, 'ADD': 0, 'REMOVE': 0, 'CHANNEL': 0,
                       'POOLING_TYPE': 0}
    ...
    def do_add_unit_mutation(self, indi):
        ...
        if type_ == 2:
            ...
            if num_exist_pool_units > StatusUpdateTool.get_pool_limit()[1] - 1:
                type_ = 1
                self.log.info(
                    'The added unit is changed to CONV because the existing number of POOLING exceeds %d, limit size:%d' % (
    ...
    def do_modify_conv_mutation(self, indi):
        ...
        if len(conv_index_list) == 0:
            ...
            if indi.units[conv_index_list[selected_index]].in_channel != channel_list[index_]:
                indi.reset_acc()
                if selected_index > 0:
                    self.log.info('Unit at %d changes its input channel from %d to %d' % (
                        conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].in_channel,
                        channel_list[index_]))
                    indi.units[conv_index_list[selected_index]].in_channel = channel_list[index_]
                    self.log.info('Due to above, the unit at %d should change its output channel from %d to %d' % (
                        conv_index_list[selected_index - 1],
                        indi.units[conv_index_list[selected_index - 1]].out_channel,
                        channel_list[index_]))
            ...
            if indi.units[conv_index_list[selected_index]].out_channel != channel_list[index_]:
                indi.reset_acc()
                self.log.info('Unit at %d changes its out channel from %d to %d' % (
                    conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].out_channel,
                    channel_list[index_]))
                ...
                if selected_index < len(conv_index_list) - 1:
                    self.log.info('Due to above, the unit at %d should change its input channel from %d to %d' % (
                        conv_index_list[selected_index + 1], indi.units[conv_index_list[selected_index + 1]].in_channel,
                        channel_list[index_]))
                    indi.units[conv_index_list[selected_index + 1]].in_channel = channel_list[index_]
                else:
                    self.log.info(
                        'Unit at %d is the last unit in the individual, therefore no need to change the input channel of the next unit')
            else:
                self.log.info('Unit at %d changes its out channel from %d to %d' % (
                    conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].out_channel,
                    channel_list[index_]))
