...
class Utils(object):
    ...
    @classmethod
    def generate_all(cls, indi, motif_str, test=False):
        ...
        if not test:
            ...
            file_name = '%s/hierarchical_representations_%s.py' % (os.path.join(os.path.dirname(get_transfer_local_path()), 'example'), indi.id)
        file_name = cls.path_replace(file_name)
